...
func declareVictory():
	...
	currRoot.add_child(sender)
	var overworldNode = currRoot.get_node("ChaseMinigame")
...
func restart():
	...
	dy = abs(ethan.position.y - you.position.y)
	phase = 0
	
func _on_slider_minigame_missed_timing_target():
	if phase < 2 && progress > 0:
		var step = punishStep
		if phase == 1:
			step *= -1
		progress += step
